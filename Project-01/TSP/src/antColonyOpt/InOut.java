...
import antColonyOpt.Tsp.problem;
...
public class InOut {
	...
	static int restart_iteration; /*
									 * remember iteration when restart was done
									 * if any
									 */
	...
	static double branch_fac; /*
								 * If branching factor < branch_fac => update
								 * trails
								 */
	...
	static int restart_found_best;/*
									 * iteration in which restart-best solution
									 * is found
									 */
	...
	static void set_default_as_parameters() {
		...
		Ants.nn_ants = 20; /*
							 * number of nearest neighbours in tour construction
							 */
		Ants.alpha = 1.0;
		Ants.beta = 2.0;
		Ants.rho = 0.5;
	 ...
	 */
	{
		...
		if (stat_report != null) {
			for (i = 0; i < Tsp.n; i++) {
				printToFile(stat_report,
						Ants.best_so_far_ant.tour[i] + " " + Tsp.instance.nodeptr[Ants.best_so_far_ant.tour[i]].x + " "
	 ...
	 */
	{
		...
		/*
		 * @Harllan - Aqui é feito a leitura do arquivo tsp
		 */
		try {
			if (name_buf.contains("brazil")) {
				distance_type = Distance_type.EXPLICIT;
				Tsp.n = 58;
				Tsp.instance.n = Tsp.n;
				Tsp.instance.name = "brazil58";
			} else {
